...
C9 <- C9[!duplicated(C9$Gene.Symbol),]
sals <- read.csv("sALSrankeduniqueresult.csv")
sals <- sals[!duplicated(sals$Gene.Symbol),]
ftld <- read.csv("FTLDrankeduniqueresult.csv")
ftld <- ftld[!duplicated(ftld$Gene.Symbol),]
vcp <- read.csv("VCPrankeduniqueresult.csv")
vcp <- vcp[!duplicated(vcp$Gene.Symbol),]
...
setwd("/users/clairegreen/Documents/PhD/TDP-43/TDP-43_Code/Results/Collaborations/EL_TDP-43/")
...
phfibgen <- phfib$hgnc_symbol

 
genelist <- readLines("~/Documents/PhD/TDP-43/TDP-43_Code/Results/GeneExpression/FoldChangeResults/Filtered_upanddown.txt")
...
genelist <- Reduce(intersect, list(rownames(subsetC9), rownames(subsetsals), rownames(subsetftld), rownames(subsetvcp), 
                                   ...
                                   rownames(subsetcbftld), rownames(subsetphfib)))
